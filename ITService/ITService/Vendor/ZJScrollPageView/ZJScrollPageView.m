...
@property (strong, nonatomic) UIImageView *shadowImageView;          //光影效果
...
- (ZJScrollSegmentView *)segmentView {
    if (!_segmentView) {
        ...
        if(weakSelf.segmentStyle.isShowShadow) {
...
- (UIImageView *)shadowImageView {
    if(!_shadowImageView) {
        CGFloat width = 14.0f * self.segmentStyle.segmentHeight / 56;
        _shadowImageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.bounds.size.width - width, 0, width, self.segmentStyle.segmentHeight)];
        _shadowImageView.contentMode = UIViewContentModeScaleAspectFit;
        _shadowImageView.image = [UIImage imageNamed:@"ic_title_tail"];
